...
public class DeleteAccountDetails {
    ...
    public Accounts deleteAccountDetails(Long AccountNumber){
        return accountsRepository.deleteByAccountNumber(AccountNumber).orElseThrow(() ->
                new AccountDetailsNotFoundException("Account not found with ID" + AccountNumber));
    }

    public Customer deleteCustomerDetails(Long customerId){
        return customerRepository.deleteByCustomerId(customerId).orElseThrow(()->
